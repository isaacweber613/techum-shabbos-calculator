...
every retraction is recorded there with dated rev notes (currently rev. 14). Isaac's
...
- Default profile Mishna Berurah/Ashkenazi: R' Chaim Naeh 48 cm amah, karpef ON (Rema/MB
  ...
  provisional 6-footprint city proxy (not MB 398:38 itself); rav review records the actual
  3-chatzeros × 2-houses, 50-resident, other attested, or negative basis with evidence.
...
- Multipolygon relations are exploded into pseudo-buildings per outer ring (slightly
  inflates footprint counts — flagged because it can affect the provisional 6-count proxy).
